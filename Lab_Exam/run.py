...
def fill_na(X):
    ...
    for i in range(X.shape[1]):
        ...
        if is_null.any():
            ...
            most_freq = mode(X.loc[~is_null, col_name])[0][0]  # <- Replace this line
            print("Most Frequent Elements = ", most_freq)
            # Replace N/A entries with most_common value
            # Hint: slice DataFrame with X.loc[???]
            # Hint: make use of is_null when slicing
            X.loc[is_null, col_name] = most_freq  # <- Replace this line
            print("X Values is: ", X)
    return X
...
def embed_categories(X):
    ...
    while c_feature < n_features:
        ...
        if current_type == dtype(object):
            ...
            dumm = get_dummies(X.loc[:, col_name])  # <- Replace this line
            print("The Dummies Values are: ", dumm)
            ...
            X.drop(col_name, axis=1, inplace=True)  # <- Replace this line
            ...
            X = concat((X, dumm), axis=1)  # <- Replace this line
            print("Value of X after Transformation is :", X.as_matrix)
            n_features -= 1
        else:
            c_feature += 1
...
print("Percentage Gain: ", (s2 - s1) * 100, "%")           # You should observe gain around 1-3%
